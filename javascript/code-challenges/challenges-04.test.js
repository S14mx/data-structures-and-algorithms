...
const sortMeetingsByDay = (arr) => {
  return arr.sort((a, b) => weekDays[a.dayOfWeek] < weekDays[b.dayOfWeek] ? -1 : 1);
};
...
const getMeetingDuration = (meeting) => {
  return (parseInt(meeting.end) - parseInt(meeting.start));
};

const sortSchedule = (arr) => {
  const timeSortedArr = arr.sort((a, b) => getMeetingDuration(a) < getMeetingDuration(b) ? -1 : 1)
    .sort((a, b) => parseInt(a.start) < parseInt(b.start) ? -1 : 1);

  return sortMeetingsByDay(timeSortedArr);
};

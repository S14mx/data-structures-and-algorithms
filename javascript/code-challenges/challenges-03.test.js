...
const typeNum = (arr) => {
  return arr.filter(arrItem => !(isNaN(arrItem)));
};
...
const containsAnd = (arr) => {
  return arr.filter(arrItem => arrItem.includes('and'));
};
...
const getStatName = (arr, minBaseStat) => {
  const stats = arr.filter(obj => obj.baseStat > minBaseStat);
  return stats.map(newObj => newObj.stat.name);
};

...
const howMuchPencil = (str) => {
  let result = [];
  let numOfletters = str.split('');
  for (let i = 0; i <= numOfletters.length; i++) {
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    slicedItems.push(recipe.ingredients[i].slice(recipe.ingredients[i].indexOf(' ') + 1));
    result.push(slicedItems[i].slice(slicedItems[i].indexOf(' ') + 1));
  }
  return result;
};
